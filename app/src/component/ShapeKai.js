...
export default class ShapeKai extends Component {
  ...
  render() {
    ...
    const dataShape = paint === "Animals" ? this.props.paintStore.getPointStoreAnimals(data.number, this.props.name):this.props.paintStore.getPointStoreObjects(data.number, this.props.name);
    const dataPath = this._CreatePath(dataShape.path);
    if (dataPath != null)
    {
      return (
        <G  onMoveShouldSetResponder={() => true} onStartShouldSetResponder={() => true} onResponderMove={(e) => this._Move(e, data)} onResponderGrant={(e) => this._Press(e, data)} onResponderRelease={(e) => this._Release(e, data, dataPath)}  >
          <Defs>

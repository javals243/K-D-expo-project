  ...
  async componentDidMount() {
    ...
    ANIMALS_DATA.map((item, index) =>{
      ...
      while(!isPass)
      {
        ...
        if((item.size.width * nScale > sizeWidth(90.5)) || (item.size.height * nScale > sizeHeight(40.5)))
        {
            isPass = true;
            break;
        }
      ...
      item.data.map((shape, shapeIndex) =>{
        AsyncStorage.getAllKeys((err, keys) => {
          ...
          if ((keys.findIndex(k => k === keyName)) == -1)
          {
            AsyncStorage.setItem(keyName, JSON.stringify({name: keyName, listDataStore: []}));
            this.props.paintStore.pointAnimals[index].point[shapeIndex].listData = [];
          }
          else
          {
            AsyncStorage.getItem(keyName, (err, result) => {
              this.props.paintStore.pointAnimals[index].point[shapeIndex].listData = JSON.parse(result).listDataStore;
            });
          }
        });
    ...
    OBJECTS_DATA.map((item, index) =>{
     ...
     item.data.map((shape, shapeIndex) =>{
      AsyncStorage.getAllKeys((err, keys) => {
        ...
        {
          AsyncStorage.setItem(keyName, JSON.stringify({name: keyName, listDataStore: []}));
          this.props.paintStore.pointObjects[index].point[shapeIndex].listData = [];
        }
        else
        {
          AsyncStorage.getItem(keyName, (err, result) => {
            this.props.paintStore.pointObjects[index].point[shapeIndex].listData = JSON.parse(result).listDataStore;
          });
        }
      });
